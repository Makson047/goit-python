...
if os.path.exists(os.path.join(os.path.dirname('.'), 'AddressBook.dat')):
    dict_phones = AddressBook.read_from_file('AddressBook.dat')
else:
...
def show_all_handler(*args):
    ...
    for contact in next(page):
        contacts_book += f'\n{contact}'
    return contacts_book


@command_handler
def find_contact_handler(text: str):
    answer_records = ''
    for record in dict_phones.find_contact(text):
